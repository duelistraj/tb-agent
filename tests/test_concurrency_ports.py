...
def test_port_ranges_are_persisted_distinct_and_recoverable(
    tmp_path: Path,
    ...
    monkeypatch.setattr(allocator, "_range_available", lambda _start, _end: True)
    first_run = uuid4()
    second_run = uuid4()
    ...
    monkeypatch.setattr(restarted, "_range_available", lambda _start, _end: True)
...
def test_worker_environment_is_run_specific(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
